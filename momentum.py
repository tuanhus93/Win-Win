...
def calculate_rsi(close_bars , stock):		#####        added by Joey         ######
...
def get_m(m_signal, rsi_dict, s):
	# -1:sell; 0: neutral; 1:buy
	for s in stocklist:
		s_rsi = rsi_dict[s]
		if s_rsi[-2]<30 and s_rsi[-1]>30 and s_rsi[0]>30:
			m_signal[s] = 1
		elif s_rsi[-2]>70 and s_rsi[-1]<70 and s_rsi[0]<70:
			m_signal[s] = -1
		else:
			m_signal[s] = 0
...
def main(argv):
	...
	rsi_dict = {} 				#####        added by Joey         ######
	m_signal = {}				#####        added by Joey         ######
	for s in stocklist:			#####        added by Joey         ######
		m_signal[s] = 0			#####        added by Joey         ######
